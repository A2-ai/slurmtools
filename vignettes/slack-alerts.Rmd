...
**Need to download slack_notifier and get on path**

# Cut me some Slack

There is also functionality to pair `nmm` with
[slack_notifier](https://github.com/a2-ai/slack_notifier) and get
messages sent directly to you via a slack bot. This requires an
additional `slack_notifier/config.yaml` file that contains the slack bot
OAuth token like so:

``` slack_notifier/config.yaml
SLACK_OAUTH_TOKEN: "supersecretslackoauthtoken"
```

Again, we need to update the `1001.toml` file to get slack notifications
