---
title: "slurmtools-vignette"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{slurmtools-vignette}
...
We again get a 0 exit code status and now instead of using `slurmtools::get_slurm_jobs()` to monitor the job, we can rely on the new notifications we just set up. ![NONMEM job starting ntfy alert](../inst/extdata/images/ntfy_starting.png)

and when the run finished we get another notification: ![NONMEM Job finished ntfy alert](../inst/extdata/images/ntfy_finished.png)
...
With `alert = 'Slack'` and `email` set in the config.toml file `nmm` should send slack notifications directly to you when a NONMEM run starts and it will reply to that message with notifications if any gradients hit 0 and when the run finishes it checks if all -1E9 lines are present in the .ext file and gives another message about any parameters that hit 0 gradient. ![slack notifications from nmm](../inst/extdata/images/nmm_slack_notifications.png)

```{r, include = FALSE}
#cancelling any running nonmem jobs
submitted_job_id3 <- submission_nmm$stdout
submitted_job_id3 <- strsplit(submitted_job_id3, "\n")[[1]]
submitted_job_id3 <- tail(strsplit(submitted_job_id3, " ")[[1]], 1)

state <- slurmtools::get_slurm_jobs() %>% 
  dplyr::filter(job_id == submitted_job_id3) %>% 
  dplyr::pull("job_state")
