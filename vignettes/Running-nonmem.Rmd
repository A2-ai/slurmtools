...
-   `bbi_exe_path` is the absolute path to `bbi`, resolved at submit time via `Sys.which("bbi")` so the rendered script names the exact binary that will run. Submission errors early if `bbi` cannot be found; point at a specific binary with `slurm_template_opts = list(bbi_exe_path = "/path/to/bbi")`
...
Here is a modified template file that adds a `JOBID=$SLURM_JOBID` and some ntfy calls. To get a notification we can supply `submit_slurm_job` with an `ntfy` variable via `slurm_template_opts`. I'll use `ntfy = "ntfy_demo"` for this.
