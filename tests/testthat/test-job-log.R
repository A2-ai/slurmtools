...
# a Job whose logs live in a scratch directory
...
test_that("a log the handle does not know about is reported as such", {
...
test_that("slurm_job_log needs the handle and checks its arguments", {
  expect_error(slurm_job_log(2051), "must be the Job returned by submit_slurm_job()", fixed = TRUE)
  job <- a_logged_job()
